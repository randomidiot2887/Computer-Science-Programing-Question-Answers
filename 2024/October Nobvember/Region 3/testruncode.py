...
import time
sttime = time.time()
for i in range(int(input("Input the number of times to repeat said code: - "))):
    tstime = time.time()
    ...
    for Member in range(len(PickerName)):
        ...
        while True:
            try:
                num = r.uniform(1, 10); inp = f"{num},{num}"
                weigh1, weigh2 = map(float, inp.split(",")) ; #print(("For example, '30,30'\n: - "))
    ...
    ttime = time.time(); print(f"{round((ttime-tstime), 1)}s, {i} times done")
print(f"\n\n\n\n\n\n\n\n\n\n\n\n--------------------------------\nTest Succesful\n------------------------------\nlooped x times. we win.")

#The looped x times is a joke. its not retail code. this is a test
etime = time.time()
print(f"{round((etime-sttime), 3)}s total")